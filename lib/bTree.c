...
void splitChild(bTree* tree, bTreeNode **x, int i, bTreeNode **y) {
    ...
    bTreeNode *z = node_init((*y)->leaf, tree);
    z->size = t-1;
...
void insert(bTree **tree, Record *record) {
    ...
    if((*tree)->next_pos == 0) {
        bTreeNode *r = node_init(true, (*tree));
        ...
        if(r->size == 2*t-1) {
            bTreeNode *s = node_init(false, (*tree));
            (*tree)->next_pos += 1;
...
Record *getPred(bTree* tree, bTreeNode *node, int idx) {
    ...
    while (curr->leaf == false) {
        //read_file(tree, curr, curr->children[curr->size]);
